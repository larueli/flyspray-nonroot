...
RUN chmod uga+x /usr/local/bin/install-php-extensions && sync && \
    ...
    install-php-extensions gd intl ctype curl phar filter exif mysqli iconv ldap mcrypt bcmath xdebug imap mongodb pgsql oauth pdo_pgsql pdo_dblib pdo_firebird pdo_odbc pdo_sqlsrv sqlsrv yaml json pdo simplexml xml tokenizer xmlwriter xmlreader pdo_mysql zip intl && \
